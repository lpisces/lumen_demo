...
class UserController extends Controller
{
  ...
  function bindingaccount(Request $request)
  {
    $ret = [];
  ...
  {
    $user = User::where('member_id', $request->input('member_id'))->select('registration_time', 'head_img', 'member_name', 'qrcode', 'Gender', 'signature', 'phone', 'status', 'bluepao', 'yellowpao', 'greenpao', 'pinkpao', 'redpao')->first();
    $user->pao = $user->bluepao + $user->yellowpao + $user->greenpao + $user->pinkpao + $user->redpao;
    return $user;
  }

  function membername(Request $request) 
  {
#    $user = User::where('member_id', $request->input('member_id'))->first();
#    if ($user == null) {
#      $ret['err_code'] = 1
#    }
  }
